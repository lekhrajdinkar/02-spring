...
- `job` : [ list of Object + action + optianal param ]
-  Flow s3:Inventary > s3:Select > S3:batch > job > processed object
- use-case/s:
...
- object: `LifeCycleRule` :
 - standard GP / Standard IA --> run `s3 analytics` take 24/28 hrs --> `CSV report`
  - gives `recommendation` to create life cycle rule to move object b/w storage classes.
...
- server side filtering : `S3-select` + `Glacier-Select`
  - s3-select filters csv row and column. more like SQL queries. 
...
- dashboard :
  - aggregated report/csv (available for 14 day, once generated) --> can publish to CW for free.
  - metric : free + $ advance (15 month)
