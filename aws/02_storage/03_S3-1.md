...
- `kms keys (single region)` 
  ...
  - encrypted (sse-s3, sse-c) --> Replicated to region-2
  - encrypted (sse-kms) : `key-region-1`
...
- `kms keys (multi-region) `
  ...
  - encrypted (sse-s3, sse-c) --> Replicated to region-2  : same as abv
  - encrypted (sse-kms) : `key-1`
