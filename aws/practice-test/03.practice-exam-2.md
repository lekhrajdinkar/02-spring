...
- high performace, scalable, cost efficeint
...
## 38. FSx
- On-prem : DFSR Distr file system replication service
- AWS : ?
  - **FSx for Window**, yes
...
## 44 : IAM
- prevent develops from  assuming AdmisAccess role
- IAM group > attach IAM permission boundary

## 45 : Store  K-V (high Ava)
- DynamoDB + ElastiCache

## 46 S3 repllication with KMS
- use multi-region KMS
- S3:batch for copy existing object.

## 47

## 48 Linus Instance > mount FS > 
- file R/W frequesntly
- then infrequently
- which one : **S3:Inteli**, Glacier, EFS, Luster

## 49 RDS(MySQL) : Storage - AutoScale (with less Admin)
- Enable RDS auto-scale, has to create CW:alarm
...
## 59 Transient gateway with resource sharing
